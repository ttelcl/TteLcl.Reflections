...
let usage focus =
  ...
  if showSection "dot" then
    cp "\fographops dot \fg-i \fcinputfile \f0[\fg-o \fcoutputfile\f0] [[\fg-ports\f0]|\fg-noports\f0|\fg-prop \fcproperty\f0|\fg-cluster\f0] [\fg-colorize \f0[\fonone\f0|\foports\f0]] [\fg-lr\f0]"
  ...
  if showDetails "dot" then
    ...
    cp "  \fg-noports\fx\f0            Do not create separate subgraphs"
