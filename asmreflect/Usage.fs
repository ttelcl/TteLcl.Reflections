...
let usage focus =
  ...
  cp ""
  cp "\foasmreflect typegraph\f0 {\fg-a \fcseed.[dll|exe]\f0} {\fg-p \fcassembly\f0} [\fg-o \fcoutput.json\f0] <flags>"
  ...
  cp "\fg-return\fx\fx                Follow method return type edges"
  cp "\fg-args\fx\fx                  Follow method argument type edges"
  cp "\fg-event\fx\fx                 Follow event type edges"
  cp "\fg-ctor\fx\fx                  Follow constructor argument edges"
